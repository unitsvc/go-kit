	...
	for {
		...
		case data := <-c.msgChan:
			...
			if _, err := c.Conn.Write(data); err != nil {
				fmt.Println("发送数据错误，", err, "写连接退出！")
				return
			}
			// fmt.Printf("发送数据成功！data = %+v\n", data)
		case data, ok := <-c.msgBuffChan:
			if ok {
				// 有数据要写给客户端
				if _, err := c.Conn.Write(data); err != nil {
					fmt.Println("发送缓冲数据错误：", err, " 写连接退出！")
					return
				}
			} else {
				fmt.Println("消息缓冲通道被关闭")
				break
			}
		case <-c.ctx.Done():
			return
	...
	for {
		...
		default:
			...
			if _, err := io.ReadFull(c.Conn, headData); err != nil {
				fmt.Println("读head消息错误，", err)
				return
			}
			// fmt.Printf("读headData %+v\n", headData)

			// 拆包，得到msgid和datalen放在msg中
			msg, err := dp.Unpack(headData)
			if err != nil {
				fmt.Println("head拆包错误，", err)
				return
			}
...
func (c *Connection) SendMsg(msgId uint32, data []byte) error {
	...
	if err != nil {
		fmt.Println("封包错误，msgId = ", msgId)
		return errors.New("封包错误")
	}
